...
controller.listar = async (req, res) => {
   ...
   if(Object.keys(req.query).length > 0) { // Se houver query string
      ...
      try {
         ...
         const lista = await ItemCompra.find().populate('compra')
         .populate('produto')
...
controller.filtrarCompra = async (req, res) => {
   let id = req.params.id

   try {
      // Só os itens de venda de uma determinada venda
      const lista = await ItemCompra.find({compra: id})
         .populate('produto')
         .populate('compra')
      res.send(lista)
   }
...
controller.obterUm = async (req, res) => {
   ...
   try {
      ...
      const obj = await ItemCompra.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      ...
      const obj = await ItemCompra.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      const id = req.body._id
      const obj = await ItemCompra.findByIdAndDelete(id)
      if (obj) {
         res.status(204).end()
      }
      else {
...
async function busca(req, res) {
   ...
   try {
      const lista = await ItemCompra.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
