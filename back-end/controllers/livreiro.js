const Livreiro = require('../models/Livreiro')

const controller = {} // Objeto vazio

controller.novo = async (req, res) => {
   try {
      await Livreiro.create(req.body)
...
controller.listar = async (req, res) => {
   ...
   if(Object.keys(req.query).length > 0) { // Se houver query string
      ...
      try {
         ...
         const lista = await Livreiro.find()
...
controller.obterUm = async (req, res) => {
   ...
   try {
      ...
      const obj = await Livreiro.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      ...
      const obj = await Livreiro.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      const id = req.body._id
      const obj = await Livreiro.findByIdAndDelete(id)
      if (obj) {
         res.status(204).end()
      }
      else {
...
async function busca(req, res) {
   ...
   try {
      const lista = await Livreiro.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
